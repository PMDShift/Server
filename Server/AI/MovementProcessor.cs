...
namespace Server.AI
{
    public class MovementProcessor
    {
            ...
            try
            {
                ...
                if (map.Tile[player.X, player.Y].Type == Enums.TileType.KeyOpen)
                {
                    ...
                    if (map.Tile[X, Y].Type == Enums.TileType.Key & map.Tile[X, Y].DoorOpen == false)
                    {
                        ...
                        {
                            hitlist.AddPacketToMap(map, PacketBuilder.CreateBattleMsg("A path was opened!", Text.WhiteSmoke));
                        }
                        else
                        {
                            hitlist.AddPacketToMap(map, PacketBuilder.CreateBattleMsg(map.Tile[player.X, player.Y].String1.Trim(), Text.WhiteSmoke));
                        }
        ...
        public static bool WillCharacterSeeDestination(IMap map, ICharacter viewer, IMap targetMap, Enums.MapID targetMapID, int targetX, int targetY)
