...
namespace Server.Players
{
    public partial class Player
    {
        ...
        public void AddToRequested(MapNpc npc)
        {
        ...
        public void BeginTempStatMode(int level, bool keepMoves, int slot)
        {
            PacketHitList hitlist = null;
            PacketHitList.MethodStart(ref hitlist);
            ...
            using (DatabaseConnection dbConnection = new DatabaseConnection(DatabaseID.Players))
            {
                if (Team[slot].Loaded)
                {
                    Team[slot].Save(dbConnection);
                }
            }
            // Now that the team is saved, change the levels!
            if (Team[slot].Loaded)
            {
                Team[slot].InTempMode = true;
                Team[slot].Level = level;
                Team[slot].Exp = 0;
                Team[slot].CalculateOriginalStats();
                Team[slot].HP = Team[slot].MaxHP;
                if (!keepMoves)
                {
                    for (int move = 0; move < Team[slot].Moves.Length; move++)
                    {
                        Team[slot].Moves[move] = new RecruitMove();
                    }
                    Team[slot].GenerateMoveset();
                }
            }
